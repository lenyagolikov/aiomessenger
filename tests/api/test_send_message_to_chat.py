...
@pytest.mark.parametrize('params, data', [({'user_id': 'user'}, {'message': 'Hello'}),
...
async def test_successful_send_message_to_chat(api_client, params, data):
    resp = await api_client.post('/v1/chats/{chat_id}/messages', params=params, json=data)
...
@pytest.mark.parametrize('params, data', [({'user_i': 'user'}, {'message': 'Hello'}),
